...
def AStarSearch(G: Graph,start,end, criteria = None, verbose = False, reached = True):
    """
    A* Search algorithm to find a path from start to end node in a graph.
    ...
    while len(pq) > 0:
        ...
        if cost > 0: cost -= G.h(cur)
        if verbose: print(f'Out: {cur} g: {cost:<3} h: {G.h(cur):<3} f: {cost+G.h(cur):<3}')
        out.append(cur)
        if cur == end:
            if verbose: print(f'\nOut order: {" ".join(out)}')
            return path
